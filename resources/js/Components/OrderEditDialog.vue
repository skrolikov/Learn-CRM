<template>
  <Dialog header="Редактировать заказ" v-model:visible="localVisible" :modal="true" :closable="false"
    style="width: 600px">
    <div class="p-fluid">
      <!-- Секция: Клиент -->
      ...
      <div class="p-col-12">
        <Panel header="Клиент" class="mb-3">
          <div class="p-field-radiobutton">
            <RadioButton inputId="existingClient" v-model="clientMode" value="existing" class="mr-2" />
            <label for="existingClient" class="p-radio-label">Использовать существующего</label>
          </div>
          <div class="p-field-radiobutton">
            <RadioButton inputId="newClient" v-model="clientMode" value="new" class="mr-2" />
            <label for="newClient" class="p-radio-label">Создать нового</label>
          </div>

          <!-- Когда выбираем уже существующего клиента -->
          <div v-if="clientMode === 'existing'" class="p-field mt-3">
            ...
            <Select id="clientDropdown" :options="clients" v-model="selectedClientId" optionLabel="name"
              optionValue="id" placeholder="Выберите клиента" fluid />
          </div>

          <!-- Когда создаём нового клиента -->
            ...
            <div class="p-col-12 p-md-6">
              <IftaLabel>
                <Select id="deviceType" :options="deviceTypes" v-model="deviceData.device_type_id" optionLabel="name"
                  optionValue="id" placeholder="Выберите тип" fluid />
            ...
            <div class="p-col-12 p-md-6 mt-2">
              <IftaLabel>
                <Select id="deviceModel" :options="deviceModels" v-model="deviceData.device_model_id" optionLabel="name"
                  optionValue="id" placeholder="Выберите модель" fluid />
            ...
            <div class="p-col-12 p-md-6 mt-2">
              <IftaLabel>
                <Select id="deviceBrand" :options="deviceBrands" v-model="deviceData.device_brand_id" optionLabel="name"
                  optionValue="id" placeholder="Выберите бренд" fluid />
            ...
            <div class="p-col-12 p-md-6 mt-2">
              <IftaLabel>
                <Select id="deviceColor" :options="deviceColors" v-model="deviceData.device_color_id" optionLabel="name"
                  optionValue="id" placeholder="Выберите цвет" fluid />
...
import { Divider, IftaLabel, Panel, Select } from 'primevue';
...
const onUpdate = async () => {
  try {
    ...
    // Формируем полезную нагрузку для обновления заказа
    const orderPayload = {
      client_id: clientId,
